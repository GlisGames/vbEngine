...
RLAPI void SetWindowRenderSize(int width, int height);            // Set the window render size
...
RLAPI int GetMonitorMaxWidth(int monitor);                           // Get specified monitor width (max available by monitor)
RLAPI int GetMonitorMaxHeight(int monitor);                          // Get specified monitor height (max available by monitor)
...
RLAPI void BeginTextureModePro(RenderTexture2D target);              // Begin drawing to render texture
...
RLAPI void DrawTextureFont(Texture2D texture, Rectangle source, Rectangle dest, Vector2 origin, float rotation, Color tint);
