...
RenderTexture2D LoadRenderTexture(int width, int height)
{
    ...
    if (target.id > 0)
    {
        ...
        // Create color texture (default to RGBA)+ //GenImageColor(width, height, BLANK).data
...
void DrawTextureFont(Texture2D texture, Rectangle source, Rectangle dest, Vector2 origin, float rotation, Color tint)
{
    // Check if texture is valid
    if (texture.id > 0)
    {
