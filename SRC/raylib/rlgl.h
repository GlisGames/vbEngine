...
RLAPI void rlEnableMultisampling(void);                   // 
RLAPI void rlDisableMultisampling(void);                  // 
...
// Textures management
RLAPI unsigned int rlLoadFontAtlas(int width, int height);
...
RLAPI void *rlReadTexturePixelsRect(Texture2D texture, Rectangle rect);
...
void rlEnableMultisampling(void)
{
#ifndef PLATFORM_WEB
    glEnable(GL_MULTISAMPLE);
#endif
}
void rlDisableMultisampling(void)
{
#ifndef PLATFORM_WEB
...
unsigned int rlLoadFontAtlas(int width, int height)
{
    unsigned int id = 0;
    /*
    glBindTexture(GL_TEXTURE_2D, 0);
...
// Read texture pixel data
void* rlReadTexturePixelsRect(Texture2D texture, Rectangle rect)
{
    void* pixels = NULL;

    unsigned int fboId = rlLoadFramebuffer(rect.width, rect.height);
    ...
    // We read data as RGBA because FBO texture is configured as RGBA, despite binding another texture format
    pixels = (unsigned char*)RL_MALLOC(rlGetPixelDataSize(rect.width, rect.height, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8));
