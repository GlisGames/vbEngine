...
void SetWindowRenderSize(int width, int height)
{
...
int GetMonitorMaxWidth(int monitor)
{
#if defined(PLATFORM_DESKTOP)
    int monitorCount;
    GLFWmonitor** monitors = glfwGetMonitors(&monitorCount);
...
// Get selected monitor width (max available by monitor)
int GetMonitorMaxHeight(int monitor)
{
#if defined(PLATFORM_DESKTOP)
    int monitorCount;
    GLFWmonitor** monitors = glfwGetMonitors(&monitorCount);
...
// Initializes render texture for drawing
void BeginTextureModePro(RenderTexture2D target)
{
    rlDrawRenderBatchActive();      // Update and draw internal render batch
    ...
    // NOTE: Configured top-left corner as (0, 0)
    rlOrtho(0, target.texture.width, target.texture.height, 0, 0.0f, 1.0f);
    //rlOrtho(0, target.texture.width, 0, target.texture.height, 0.0f, 1.0f);
...
void BeginScissorMode(int x, int y, int width, int height)
{
    ...
    if (CORE.Window.flags & FLAG_FIXED_RENDER_SIZE)
    {
        ...
        rlScissor(((float)x / rapw), (float)CORE.Window.currentFbo.height - (((float)y / raph) + ((float)height / raph)), ((float)width / rapw), ((float)height / raph));
    }
    else
