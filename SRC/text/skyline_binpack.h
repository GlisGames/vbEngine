...
public:
	...
	binRect Insert(int width, int height);
...
private:
	...
	binRect InsertBottomLeft(int width, int height);

	binRect FindPositionForNewNodeBottomLeft(int width, int height, int &bestHeight, int &bestWidth, int &bestIndex) const;

	bool RectangleFits(int skylineNodeIndex, int width, int height, int &y) const;

	void AddSkylineLevel(int skylineNodeIndex, const binRect &rect);
