...
binRect SkylineBinPack::Insert(int width, int height)
{
...
binRect SkylineBinPack::InsertBottomLeft(int width, int height)
{
	int bestHeight;
	int bestWidth;
	int bestIndex;
	//width++;
	//height++;
	binRect newNode = FindPositionForNewNodeBottomLeft(width, height, bestHeight, bestWidth, bestIndex);
...
binRect SkylineBinPack::FindPositionForNewNodeBottomLeft(int width, int height, int &bestHeight, int &bestWidth, int &bestIndex) const
{
	binRect newNode;
...
{
	int x = skyLine[skylineNodeIndex].x+1;
	if (x + width > binWidth)
		return false;
	int widthLeft = width;
...
void SkylineBinPack::AddSkylineLevel(int skylineNodeIndex, const binRect&rect)
{
	SkylineNode newNode;
	newNode.x = rect.x;
