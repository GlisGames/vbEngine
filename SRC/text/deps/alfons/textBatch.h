...
public:
    ...
    void drawTransformedShape(const Font& font, const Shape& shape, const glm::vec2& position,
                              float scale, bool _newLine = false, LineMetrics& metrics = NO_METRICS);

    /* Use current QuadMatrix for transform */
    inline void drawTransformedShape(const Font& font, const Shape& shape, float x, float y,
                                     float scale, bool _newLine, LineMetrics& metrics = NO_METRICS) {
        drawTransformedShape(font, shape, glm::vec2(x, y), scale, _newLine, metrics);
    }
    ...
    float draw(const LineLayout& line, const LineSampler& path, bool _newLine = false,
